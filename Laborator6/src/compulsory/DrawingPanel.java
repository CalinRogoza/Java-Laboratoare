...
public class DrawingPanel extends JPanel {
    ...
    private void drawShape(int x, int y) {
        int radius = new Random().nextInt(200); //generate a random number
        int sides = (int) frame.configPanel.sidesField.getValue(); //get the value from UI (in ConfigPanel)
        int color = frame.configPanel.colorCombo.getSelectedIndex(); //create a transparent random Color.
        String shapes = (String) frame.configPanel.shapesField.getValue();
        Color drawingColor;
        ...
        if (1 == color) { //"Black"
            ...
            drawingColor = new Color(red, green, blue, 200);
        }

        if (shapes == "Random Polygon") {
